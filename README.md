...
4.  **High Performance**: Use of Camel-Boot to instantiate & run the Camel application natively in the JVM.  With Spring-Boot an application server or run-time is not required to run the Camel application.  As a result, the application is extremely light weight and delivers better overall throughput and run-time performance.  
